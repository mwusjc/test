...
class CPrivateLabelAdmin extends CSectionAdmin{

  /** comment here */
  function CPrivateLabelAdmin() {
	$this->CSectionAdmin();
  }

  /** comment here */
  function registerClasses() {
	$this->mClasses[] = array("Users", "CPrivateLabel");
  }


  /** comment here */
  function display() {
    STitle::set("Manage Private Labels");
	SAccess::enforce("manage_private_labels");
	$sql = "SELECT * ".
			" FROM private_labels a WHERE 1=1 ".
			" ##CRITERIA## ";
...
//	$vSmart->addLFilter("a.GroupID", "User Group", $this->mDatabase->getAll2("select ID, Txt AS Category from cms_user_groups order by Txt ASC"), 1);
	...
	$vSmart->mColsWidths = array("10px", "10px", "700px", "90px");
  ...
  function displayEdit($pItemID = 0) {
	SAccess::enforce("manage_private_labels");
	  $this->nav($pItemID, "edit", "private_labels");
	$vUser = new CPrivateLabel($pItemID);
  ...
  function delete($pUserID) {
	SAccess::enforce("manage_private_labels");
	  $vUser = new CPrivateLabel($pUserID);
	  $vUser->delete();
