...
class CMessage extends CDBContent {

	var $section = "Texts";
	var $parent = "CMessageAdmin";
	var $table = "cms_messages";


	function CMessage($pPageID) {
	  if (!$pPageID) $this->CDBContent($pPageID);
	  else {
		  $this->CDBObject();
		  if ((string)intval($pPageID) == $pPageID) {
			$this->mRowObj = $this->mDatabase->getRowObj("select * from cms_messages where id ='$pPageID'");
		  } else {
			$this->mRowObj = $this->mDatabase->getRowObj("select * from cms_messages where name ='".addslashes($pPageID)."'");
		  }
	  }
	}
...
//	  Return nl2br($this->mRowObj->Txt);
	...
	function displayEdit() {
	  $this->resetTemplate("_common/templates/pages/edit_text.html");
